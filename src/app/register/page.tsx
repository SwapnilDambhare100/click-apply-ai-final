...
export default function Register() {
  ...
  const saveUserAndRedirect = (userName: string, userEmail: string) => {
    // Save user info to localStorage so dashboard can display it
  ...
  const handleGoogleSignIn = () => {
    setLoading(true);
    // Prompt user for their Google email since we don't have real OAuth
    ...
    saveUserAndRedirect(userName, userEmail);
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email) return;
    saveUserAndRedirect(name, email);
  };
